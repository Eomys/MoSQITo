...
def out_mid_validation(fs, signal, signal_filtered, sos_ear):
    ...
    print('\n\n_____________OUTER AND MIDDLE EAR FILTERING_____________\n')
    print('Outer & Middle Ear Filter Coefficients:\n' + str(sos_ear))

    """ GRAPHIC REPRESENTATION """
    # Frequency response 'sos'
    print('Freq. response and filtering:\n')
    ...
    plt.legend(['F. Response (sos filter)'])
    plt.xlim(20, 20000)
    plt.ylim(-25, 10)
...
def afb_validation(fs, dim, centre_freq_array, f_bandwidth_array, t_delay_array, d_coefficients_array, sb_array,
    ...
    print('Am coefficient - band-pass filter:\n' + str(am_mod_coefficient_array) + '\n')
    print('Bm coefficient - band-pass filter:\n' + str(bm_mod_coefficient_array) + '\n')
    ...
    for afb_band in range(0, 53):
        ...
        plt.semilogx(w_afb_hertz, h_afb_db,
                     label='FR band: ' + str(band_number) + ' (' + str(centre_freq_array[afb_band]) + 'Hz filter)')
    ...
    if dim == 2:
        ...
        [plt.plot(time, band_pass_signal_array[band_num, :, 0],
                  label='Band-pass signal ' + str(band_num + 1) + ' - '
                        + str(centre_freq_array[band_num]) + 'Hz)') for band_num in rep_bands]
        plt.legend()
...
def sl_validation(dim, centre_freq_array, n_array):
    ...
    print('\n\n_____________SPECIFIC LOUDNESS WITH CONSIDERATION OF THRESHOLD IN QUIET_____________\n')
    ...
    if dim == 2:
        ...
        plt.figure(figsize=(10, 5))
        [plt.semilogx(centre_freq_array, n_array[int(block_num), :, 0],
                      label='SL values, block: ' + str(int(block_num))) for block_num in range(len(n_array))]
        plt.legend()
...
def tl_validation(dim, t_array):
    ...
    """
    print('\n\n_____________TOTAL LOUDNESS_____________\n')
    ...
    if dim == 2:
        ...
        plt.ylabel('Sound pressure [sones]')
        plt.title('Total loudness contours (Left channel)')
        plt.grid(which='both', linestyle='-', color='grey')
        plt.show()
        ...
        plt.xlabel('Block number')
        plt.ylabel('Sound pressure [sones]')
        plt.title('Total loudness contours (Right channel)')
        plt.grid(which='both', linestyle='-', color='grey')
        plt.show()
        ...
        plt.xlabel('Block number')
        plt.ylabel('Sound pressure [sones]')
        plt.title('Total loudness contours (Mono channel)')
        plt.grid(which='both', linestyle='-', color='grey')
        plt.show()
