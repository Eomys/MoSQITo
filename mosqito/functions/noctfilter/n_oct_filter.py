...
def designFilters(freqDict, fs=48000, plot=False):
    ...
    if plot is True:
        ...
        freqH = 1 + ((G ** (1 / (2 * b)) - 1) / (G ** (1 / 2) - 1)) * (G ** (breakpoints) - 1)
        freqL = 1 / freqH

        plt.figure('Filter responses with {} bands from {} Hz to {} Hz'.format(np.size(freqs, 0), np.int(freqs[0, 0]), np.int(freqs[-1, 2])))
        for index in range(np.size(freqs, axis=0)):
            # Plot
...
def analyseData(filters, data, freqDict=None, plot=False):
    ...
    if plot == 1:
        ...
        if meanSignal.ndim == 1:
            ax.bar(freqs[:, 0], 20 * np.log10(meanSignal) - minMean, align='edge', bottom=minMean, width=(freqs[:, 2] - freqs[:, 0]))
        elif meanSignal.ndim == 2:
            for i in range(np.size(meanSignal, axis=1)):
                ax.bar(freqs[:, 0], 20 * np.log10(meanSignal[:, i]) - minMean, align='edge', bottom=minMean, width=(freqs[:, 2] - freqs[:, 0]))
        ax.set_xscale('log')
        plt.show()
    return meanSignal
