...
original_signal, filtered_signal, Fs = variant_filter(signal, track,
                                                      ftype, armonic_order, band)
'''
...
#RPM signal
rpm_signal, fs = load(False, 'C:/Users/josem/Desktop/INGSI/PFG/Test signals/RUN5_RunUp_60s_RPM-profile_Rec0.uff', calib = 1)
...
plt.specgram(original_signal, NFFT=NFFT,
                                  Fs=Fs,vmin=vmin, noverlap=noverlap,
                                  cmap = plt.cm.jet, window=plt.window_none)
...
ax1 = plt.subplot(gs[0])
pxx,  freq, t, cax = plt.specgram(filtered_signal, NFFT=NFFT,
                                  Fs=Fs,vmin=vmin, noverlap=noverlap,
                                  cmap = plt.cm.jet, window=plt.window_none)
plt.ylabel('Frequency [Hz]')
