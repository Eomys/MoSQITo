...
def test_tnr():
    ...
    for i in range(len(signal)):
        ...
        tnr = comp_tnr(signal[i]["is_stationary"], audio, fs, prominence=True, plot=True)        
        # Check compliance
        tst = check_compliance(tnr, signal[i])
    
    return tst
    

def check_compliance(tnr, signal):
