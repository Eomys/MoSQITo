...
def compute_level(self, nb_points=[], start=[],stop=[]):
    ...
    if nb_points!=[]:
        if type(nb_points)!=int:
            raise TypeError(
                "ERROR : Number of points should be an integer"
                )
    ...
    if start!=[] and stop!=[]:
        if type(start)!=int and type(start)!=float:
            raise TypeError(
                "ERROR : Start (in sec) should be an integer or a float "
                )

        if type(stop)!=int and type(stop)!=float:
            raise TypeError(
                "ERROR : Stop (in sec) should be an integer or a float "
                )
  
        
        if start<0 or stop<0 or start>len(self.signal.values)/self.fs or stop>len(self.signal.values)/self.fs:
            raise ValueError(
        ...
        if start == stop:
            raise ValueError(
                "ERROR : Start and stop values must be different"
                )
    ...
    if nb_points != []:
        ...
        time = Data1D(
            name="time",
            unit="s", 
            values=linspace(start, stop,num=nb_points)
        )
