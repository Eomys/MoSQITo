...
def pr_ecma_freq(spectrum, freqs, prominence=True):
    ...
    if prominence == False:
        ...
        return t_pr, np.asarray(pr, np.float32)[np.asarray(prom, bool)], np.asarray(prom, bool)[np.asarray(prom, bool)], np.asarray(tones_freqs, np.float32)[np.asarray(prom, bool)]
